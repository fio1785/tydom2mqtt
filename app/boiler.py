        ...
        if (self.mqtt is not None):
            if 'temperature' in self.attributes:
                self.mqtt.mqtt_client.publish(
                    ...
                    qos=0)
                logger.info("Boiler created / updated : %s %s temperature: %s", self.name, self.id, self.attributes['temperature'])
            ...
            if 'thermicLevel' in self.attributes:
                ...
                self.mqtt.mqtt_client.publish(
                    ...
                    qos=0)
                logger.info("Boiler created / updated : %s %s thermicLevel: %s", self.name, self.id, self.attributes['thermicLevel'])
            if 'outTemperature' in self.attributes:
                self.mqtt.mqtt_client.publish(
                    self.config['state_topic'],
                    self.attributes['outTemperature'],
                    qos=0)
                logger.info("Boiler created / updated : %s %s outTemperature: %s", self.name, self.id, self.attributes['outTemperature'])
