...
#                self.mqtt.mqtt_client.publish(self.config['mode_state_topic'], "off" if self.attributes['authorization'] == "STOP" else "heat", qos=0)
        ...
        logger.info("Boiler created / updated : %s %s current temperature: %s setpoint: %s mode: %s", self.name, self.id, self.attributes['temperature'], self.id, self.attributes['setpoint'], self.attributes['thermicLevel'])
